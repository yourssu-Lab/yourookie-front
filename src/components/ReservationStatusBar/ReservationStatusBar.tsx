...
} from './ReservationStatusBar.style.ts';
...
export const ReservationStatusBar = ({
    ...
    for (let hour = startHour; hour <= endHour; hour++) {
      for (const minute of [0, 30]) {
        ...
        const reserved = reservations.some((res) => {
          const startTime =
            new Date(res.startDateTime).getHours() * 60 + new Date(res.startDateTime).getMinutes();
          const endTime =
            new Date(res.endDateTime).getHours() * 60 + new Date(res.endDateTime).getMinutes();
  ...
  const handleSelect = (hour: number, minute: number) => {
    ...
    if (!selectionStart) {
      ...
      for (let currentTime = rangeStart; currentTime <= rangeEnd; currentTime += 30) {
        const slotHour = Math.floor(currentTime / 60);
        const slotMinute = currentTime % 60;

        const isReserved = timeSlots.some(
          (slot) =>
            slot.hour === slotHour && slot.minute === slotMinute && slot.status === 'reserved',
        );
  ...
  const isSelected = (hour: number, minute: number) => {
    return selectedSlots.some((slot) => slot.hour === hour && slot.minute === minute);
  };
                ...
                <StyledTimeSlot
                  ...
                  $status={
                    isSelected(hour, minute)
                      ? 'selected'
                      : timeSlots.find((slot) => slot.hour === hour && slot.minute === minute)
                          ?.status || 'available'
                  }
                  $isSelectionStart={
                    selectionStart?.hour === hour && selectionStart?.minute === minute
                  }
